 ...
 * paper as resources. createServer builds the configured server, and when the file is run
 * directly (npm run mcp) it is connected over stdio.
 *
...
/**
 * Build the configured MCP server with the chocobo_odds tool and the research and paper
 * resources, without connecting any transport.
 *
...
// Connect over stdio only when run directly (npm run mcp), not when imported by a test.
